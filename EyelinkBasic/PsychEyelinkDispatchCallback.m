...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Edit for AMD experiment

try
    MMTfixation = el.MMTfixation;
catch
    MMTfixation = false;
end
...
if ~MMTfixation
    ...
    try
        outerRadius = el.MMTouterRadius;
    catch
        outerRadius = 400;
    end

    try
        innerRadius = el.MMTinnerRadius;
    catch
        innerRadius = 0;
    end
    
    try
        nspikes = el.MMTnspikes;
    catch
        nspikes = 8;
    end
    
    try
        spikewidth = el.MMTspikewidth;
    catch
        spikewidth = (360/nspikes)/3; % in degrees
    end
    
    framewidth = ceil((outerRadius/cosd(spikewidth/2))-outerRadius); % pen width needed to hide and round poly corners

    try
        fixpolycoords = getMMTfixPoly(nspikes,0,spikewidth,innerRadius,outerRadius/2);
    catch
        warning('MMT fixation poly generation failed. Make sure getMMTfixPoly() is on your MATLAB path')
    end
    
    for p = 1:nspikes
        curcoords = fixpolycoords(1:3,(2*p-1):(2*p))+repmat([calxy(1) calxy(2)],[3 1]);
        Screen('FillPoly', eyewin, el.calibrationtargetcolour,curcoords);
    end
    
    Screen('FrameOval', eyewin, el.calibrationtargetcolour,...
        [calxy(1)-outerRadius/2, calxy(2)-outerRadius/2, calxy(1)+outerRadius/2, calxy(2) + outerRadius/2]',...
